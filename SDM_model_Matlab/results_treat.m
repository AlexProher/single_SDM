%%
% load('nominal.mat', 'results');
load('worstCase.mat', 'results');
...
plot(results.pid_time, results.pid(:,3),  "LineStyle","--", LineWidth=2);
hold on;
plot(results.hinf_time, results.hinf(:,3),  "LineStyle","-.", LineWidth=2);
plot(results.mu_time, results.mu(:,3),  "LineStyle",":", LineWidth=2);
grid on;
...
%%
metrics = struct();
ce.pid = trapz(results.pid_time, results.pid(:, 3).^2);
ce.hinf = trapz(results.hinf_time, results.hinf(:, 3).^2);
ce.mu = trapz(results.mu_time, results.mu(:, 3).^2);
